...
public class Zone implements Serializable {
    ...
    /* JADX WARNING: Missing block: B:22:0x0038, code skipped:
            if (r3.getType() != r8) goto L_0x003a;
     */
                ...
                if (types != null) {
                    ...
                    if (isExact && type == 255) {
                        ...
                        for (RRset addRRset : sets) {
                            sr.addRRset(addRRset);
                        }
                    } else {
                        if (!isExact) {
                            rrset = oneRRset(types, 39);
                            if (rrset != null) {
                                sr = new SetResponse(5, rrset);
                                break;
                            }
                        }
                        rrset = oneRRset(types, type);
    ...
    private void nodeToString(StringBuffer sb, Object node) {
        ...
        for (RRset rrset : sets) {
            ...
            while (it.hasNext()) {
                sb.append(new StringBuffer().append(it.next()).append(MASTNativeAdConstants.NEWLINE).toString());
            }
            it = rrset.sigs();
            while (it.hasNext()) {
                sb.append(new StringBuffer().append(it.next()).append(MASTNativeAdConstants.NEWLINE).toString());
