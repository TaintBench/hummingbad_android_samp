...
public class Name implements Comparable, Serializable {
    ...
    public Name(String s, Name origin) throws TextParseException {
        if (s.equals("")) {
            ...
            for (int i = 0; i < s.length(); i++) {
                ...
                int pos2;
                if (escaped) {
                    if (b >= (byte) 48 && b <= (byte) 57 && digits < 3) {
                        digits++;
    ...
    public Name relativize(Name origin) {
        ...
        thisR.setlabels(thisR.labels() - origin.labels());
        thisR.name = new byte[length];
        System.arraycopy(this.name, offset(0), thisR.name, 0, length);
        return thisR;
    }
